...
func UploadHandler(w http.ResponseWriter, r *http.Request) {
	// 创建 ./cmd/k8s-pod-info-exporter/config 目录
	configDir := "./cmd/k8s-pod-info-exporter/config"
	// 创建config目录
	...
	// 创建 ./cmd/k8s-pod-info-exporter/output 目录
	outputDir := "./cmd/k8s-pod-info-exporter/output"
	// 创建output目录
	...
	filePath := filepath.Join(configDir, originalFilename) // 将文件保存在 ./cmd/k8s-pod-info-exporter/config 目录中
	...
	// config 目录下文件的相对路径 ./cmd/k8s-pod-info-exporter/config
	relPath := fmt.Sprintf("./cmd/k8s-pod-info-exporter/config/%s", originalFilename)
	...
	err = cmd.Run()
	if err != nil {
		log.Printf("Error executing command 123: %v", err)
